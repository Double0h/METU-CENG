...
def main():
    r3_to_r2 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    r3_to_r2.bind(("10.10.6.2", 3002))
    r3_to_s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    s = ("10.10.3.1", 5000)
    r3_to_d = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    d = ("10.10.7.1", 5001)

    t1 = threading.Thread(target=threaded_ack, args=(r3_to_r2,))		# thread that runs server for r2 communication
    t2 = threading.Thread(target=threaded_send, args=(r3_to_s, s))		# thread that runs client for s communication
    t3 = threading.Thread(target=threaded_send, args=(r3_to_d, d))		# thread that runs client for d communication
